...
db = pymysql.connect(host='mysql',user='root',password='rootdevpassword',db='demo',charset='utf8mb4',cursorclass=pymysql.cursors.DictCursor)
...
class UserMySQLDao(UserDao):
    ...
    def add_user(cls, user: User):
        ...
        VALUE ('{user.line_user_id}','{user.line_user_pic_url}','{user.line_user_nickname}','{user.line_user_status}','{user.line_user_system_language}',
        '{user.message_files}','{user.image_files}','{user.audio_files}','{user.video_files}','{user.blocked}');
        """
        print(sql)
        try:
